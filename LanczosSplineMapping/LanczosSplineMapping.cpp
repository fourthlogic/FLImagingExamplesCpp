...
int main()
{
	...
	do
	{
		...
		CLanczosSplineMapping lanczosSplineMapping;
		...
		if(IsFail(res = lanczosSplineMapping.Load(L"MappingData.fllcz")))
		{
			...
			lanczosSplineMapping.Initialize(flpGridSize, i32Extension);
			...
			if(IsFail(res = lanczosSplineMapping.Finish()))
			{
			...
			if(IsFail(res = lanczosSplineMapping.Save(L"MappingData.fllcz")))
			{
		...
		for(int32_t y = 0; y < lanczosSplineMapping.GetRow(); ++y)
		{
			for(int32_t x = 0; x < lanczosSplineMapping.GetColumn(); ++x)
			{
				const CLanczosSplineMapping::CLanczosSplineMappingVertexInfo* pVertex = lanczosSplineMapping.GetControlPoint(CFLPoint<int32_t>(x, y));
		...
		for(int y = 0; y <= (lanczosSplineMapping.GetRow() - 1) * f64Slice; ++y)
		{
			...
			for(int x = 0; x <= (lanczosSplineMapping.GetColumn() - 1) * f64Slice; ++x)
			{
				...
				if(IsOK(lanczosSplineMapping.ConvertSourceToDestination(flpSource, flpDestination)))
				{
					...
					if(IsOK(lanczosSplineMapping.ConvertDestinationToSource(flpDestination, flpConvertedSource)))
					{
