...
int main()
{
	...
	CPatternMatchMultiSparse patternMatchMultiSparse;
	...
	do
	{
		...
		for(int64_t i64DataIdx = 0; i64DataIdx < 2; ++i64DataIdx)
		{
			...
			if(IsFail(res = patternMatchMultiSparse.Learn(arrClassName[i64DataIdx])))
			{
		...
		patternMatchMultiSparse.SetAngleBias(0.);
		...
		patternMatchMultiSparse.SetAngleTolerance(15.);
		...
		patternMatchMultiSparse.SetMaxObjectMode(CPatternMatchMultiSparse::EMaxObjectMode_Total);
		...
		patternMatchMultiSparse.SetConflictDetectionMethod();

		// 알고리즘 수행 // Execute the algorithm
		res = patternMatchMultiSparse.Execute();
		...
		int64_t i64ResultCount = patternMatchMultiSparse.GetResultCount();
