...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSrcImage.Load(L"../../ExampleImages/ChannelLinfNorm/Coord.flif")))
		{
		...
		if((res = channelLinfNorm.SetSourceImage(fliSrcImage)).IsFail()) 
			break;
		if((res = channelLinfNorm.SetDestinationImage(fliDstImage)).IsFail()) 
			break;
