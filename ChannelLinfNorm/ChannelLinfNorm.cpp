...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = arrFliImage[EImageType_Source].Load(L"../../ExampleImages/ChannelLinfNorm/Coord.flif")))
		{
		...
		CChannelLinfNorm channelLinfNorm;

		if((res = channelLinfNorm.SetSourceImage(arrFliImage[EImageType_Source])).IsFail())
			break;
		if((res = channelLinfNorm.SetDestinationImage(arrFliImage[EImageType_Destination])).IsFail())
			break;
