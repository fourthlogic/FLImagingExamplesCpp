...
int main()
{
	...
	do
	{
		...
		CPatternMatch patternMatchSave;
		...
		if(IsFail(res = patternMatchSave.Learn()))
		{
		...
		if(IsFail(res = patternMatchSave.Save(L"../../ExampleImages/Matching/Pattern Single Learn File")))
		{
		...
		CPatternMatch patternMatchLoad;

		// 데이터 Load를 진행합니다. // Proceed to load data.
		if(IsFail(res = patternMatchLoad.Load(L"../../ExampleImages/Matching/Pattern Single Learn File")))
		{
			ErrorPrint(res, "Failed to save\n");
			break;
		}
		...
		patternMatchLoad.SetAngleBias(0.);
		...
		patternMatchLoad.SetAngleTolerance(10.);
		...
		int64_t i64ResultCount = patternMatchLoad.GetResultCount();
