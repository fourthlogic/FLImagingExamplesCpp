...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = arrFliImage[EType_Source].Load(L"../../ExampleImages/OperationMatrixMultiply/Matrix0.flif")))
		{
		...
		if(IsFail(res = arrFliImage[EType_Operand].Load(L"../../ExampleImages/OperationMatrixMultiply/Matrix1.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		COperationMatrixMultiply operationMatrixMultiply;
		// Source 이미지 설정 // Set the source image
		operationMatrixMultiply.SetSourceImage(arrFliImage[EType_Source]);
		// Operand 이미지 설정 // Set the operand image
		operationMatrixMultiply.SetOperandImage(arrFliImage[EType_Operand]);
		// Destination 이미지 설정 // Set the destination image
		operationMatrixMultiply.SetDestinationImage(arrFliImage[EType_Destination]);
		// 연산 방식 설정 // Set operation source
		operationMatrixMultiply.SetOperationSource(EOperationSource_Image);
		// Overflow 처리 방식 설정 // Set Overflow Method
		operationMatrixMultiply.SetOverflowMethod(EOverflowMethod_Wrapping);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(res = operationMatrixMultiply.Execute()))
		{
