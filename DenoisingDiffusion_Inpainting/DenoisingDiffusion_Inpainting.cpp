...
{
	CDenoisingDiffusionInpaintingDL* pDD = (CDenoisingDiffusionInpaintingDL*)pParam;
...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliLearnImage.Load(L"../../ExampleImages/DenoisingDiffusion/Ball_Train.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(res = fliValidateImage.Load(L"../../ExampleImages/DenoisingDiffusion/Ball_Validation.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/DenoisingDiffusion/Ball_Source.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		// 학습할 Denoising Diffusion 모델 설정 // Set up Denoising Diffusion model to learn
		denoisingDiffusionInpaintingDL.SetModelVersion(CDenoisingDiffusionInpaintingDL::EModelVersion_FLGenNet_Diffusion_Inpainting_256);
		...
		denoisingDiffusionInpaintingDL.SetLearningGradientClippingThreshold(1.f);
		...
		autoSaveSpec.SetAutoSavePath(L"model.flddi");
		...
		denoisingDiffusionInpaintingDL.SetLearningStopCondition(L"");
		...
		hThread = (HANDLE)_beginthreadex(NULL, 0, LearnThread, (void*)&denoisingDiffusionInpaintingDL, 0, nullptr);

		while(!denoisingDiffusionInpaintingDL.IsRunning() && !g_bTerminated)
			CThreadUtilities::Sleep(1);

		int32_t i32MaxEpoch = denoisingDiffusionInpaintingDL.GetLearningEpoch();
		...
		while(true)
		{
			...
			int32_t i32MaxIteration = denoisingDiffusionInpaintingDL.GetActualMiniBatchCount();
			// 마지막 미니 배치 반복 횟수 받기 // Get the last number of mini batch iterations
			int32_t i32Iteration = denoisingDiffusionInpaintingDL.GetLearningResultCurrentIteration();
			// 마지막 학습 횟수 받기 // Get the last epoch learning
			int32_t i32Epoch = denoisingDiffusionInpaintingDL.GetLastEpoch();
			...
			denoisingDiffusionInpaintingDL.GetLearningResultAllHistory(vctCosts, vctSSIM, vctMRQ, vctValidationEpoch);
			...
			if(vctCosts.GetCount() && i32Epoch != i32PrevEpoch && i32Iteration == i32MaxIteration && i32Epoch > 0)
			{
				...
				if((vctCosts.GetCount() && i32PrevCostCount != (int32_t)vctCosts.GetCount()) || (vctSSIM.GetCount() && i32PrevValidationCount != (int32_t)vctSSIM.GetCount()))
				{
					...
					int32_t i32Step = denoisingDiffusionInpaintingDL.GetLearningValidationStep();
					CFLArray<float> flaX;
		...
		if(IsFail(res = denoisingDiffusionInpaintingDL.Execute()))
		{
