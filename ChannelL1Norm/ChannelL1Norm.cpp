...
int main()
{
	...
	{
		// 이미지 로드 // Loads image
		if(IsFail(res = arrFliImage[EImageType_Source].Load(L"../../ExampleImages/ChannelL1Norm/Coord.flif")))
		{
		...
		if((res = channelL1Norm.SetSourceImage(arrFliImage[EImageType_Source])).IsFail())
			break;
		if((res = channelL1Norm.SetDestinationImage(arrFliImage[EImageType_Destination])).IsFail())
			break;
