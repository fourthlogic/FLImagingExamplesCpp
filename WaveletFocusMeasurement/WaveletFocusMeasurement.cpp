...
int main()
{
	...
	do
	{
		...
		CWaveletFocusMeasurement waveletFocusMeasurement;

		// 기저 함수 설정 // Set Basis Function
		waveletFocusMeasurement.SetBasisFunction(CWaveletFocusMeasurement::EBasisFunction_Haar);

		// 분해 단계 설정 // Set Decomposition Level
		waveletFocusMeasurement.SetDecompositionLevel(1);

		// Source 이미지 1 설정 // Set the source1 image
		waveletFocusMeasurement.SetSourceImage(arrFliImage[EImageType_Source1]);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(res = waveletFocusMeasurement.Execute()))
		{
		...
		waveletFocusMeasurement.SetSourceImage(arrFliImage[EImageType_Source2]);
		...
		waveletFocusMeasurement.SetSourceImage(arrFliImage[EImageType_Source3]);
		...
		CFLString<wchar_t> flsLayer3 = CFLString<wchar_t>().Format(L"Score : %lf", f64Score3);
		...
		if(IsFail(res = layerSrc3.DrawTextCanvas(&CFLPoint<double>(0, 0), flsLayer3, YELLOW, BLACK, 20)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
