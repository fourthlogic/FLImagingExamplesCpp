...
int main()
{
	...
	do
	{
		...
		if((res = arrLayer[EType_Destination1].DrawTextCanvas(&flpZero, L"Destination1 Image(BitRollingLeft 1)", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}

		if((res = arrLayer[EType_Destination2].DrawTextCanvas(&flpZero, L"Destination1 Image(BitRollingLeft 7)", YELLOW, BLACK, 25)).IsFail())
		{
			ErrorPrint(res, L"Failed to draw text\n");
			break;
		}
