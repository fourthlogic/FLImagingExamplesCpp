...
{
	CMeasurementInstanceDL* pMeasurement = (CMeasurementInstanceDL*)pParam;
...
int main()
{
	...
	do 
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliLearnImage.Load(L"../../ExampleImages/MeasurementDL/Cross.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(res = fliValidationImage.Load(L"../../ExampleImages/MeasurementDL/Cross.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		CMeasurementInstanceDL measurementInstanceDL;
		...
		measurementInstanceDL.SetInterpolationMethod(EInterpolationMethod_Bilinear);

		// 학습을 종료할 조건식 설정. map값이 1 이상인 경우 학습 종료한다. metric과 동일한 값입니다.
		...
		autoSaveSpec.SetAutoSavePath(L"model.flmsidl");
		...
		hThread = (HANDLE)_beginthreadex(NULL, 0, LearnThread, (void*)&measurementInstanceDL, 0, nullptr);

		while(!measurementInstanceDL.IsRunning() && !g_bTerminated)
			CThreadUtilities::Sleep(1);

		int32_t i32MaxEpoch = measurementInstanceDL.GetLearningEpoch();
		...
		while(true)
		{
			...
			int32_t i32MaxIteration = measurementInstanceDL.GetActualMiniBatchCount();
			// 마지막 미니 배치 반복 횟수 받기 // Get the last number of mini batch iterations
			int32_t i32Iteration = measurementInstanceDL.GetLearningResultCurrentIteration();
			// 마지막 학습 횟수 받기 // Get the last epoch learning
			int32_t i32Epoch = measurementInstanceDL.GetLastEpoch();
			...
			if(i32Epoch != i32PrevEpoch && i32Iteration == i32MaxIteration && i32Epoch > 0)
			{
				...
				measurementInstanceDL.GetLearningResultAllHistory(&vctCosts, &vctMeanAP, &vctValidationEpoch, &vctRecall, &vctPrecision);
				...
				{
					int32_t i32Step = measurementInstanceDL.GetLearningValidationStep();
					CFLArray<float> flaX;
			...
			if(!measurementInstanceDL.IsRunning() && g_bTerminated)
			{
		...
		measurementInstanceDL.SetInferenceResultImage(fliResultLabelImage);

		// 알고리즘 수행 // Execute the algorithm
		if(IsFail(res = measurementInstanceDL.Execute()))
		{
