...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = arrFliImage[EViewList_Learn].Load(L"../../ExampleImages/QRCode/Learn.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(res = arrFliImage[EViewList_ModelBaseFind].Load(L"../../ExampleImages/QRCode/Find.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(res = arrFliImage[EViewList_NormalFind].Load(L"../../ExampleImages/QRCode/Find.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		CModelBasedQRCodeDecoder modelBasedQRCodeDecoder;

		// 학습 이미지 설정 // Sets the learn image.
		modelBasedQRCodeDecoder.SetLearnImage(arrFliImage[EViewList_Learn]);
		// 코드 색상 설정 // Sets the code color.
		modelBasedQRCodeDecoder.SetColorMode(EDataCodeColor_BlackOnWhite);
		...
		modelBasedQRCodeDecoder.SetSourceImage(arrFliImage[EViewList_ModelBaseFind]);
		// 디코딩 결과 개수 설정 // Sets the number of decoding results.
		modelBasedQRCodeDecoder.SetDetectingCount(EDataCodeDecoderDetectingCount_All);
		...
		CModelBasedQRCodeDecoder::CQRCodeLearnInformation qrcodeLearnInfo;
		modelBasedQRCodeDecoder.GetLearnResult(qrcodeLearnInfo);
		{
			wprintf(L"\n[Model Based Learn Result]\n");

			CFLQuad<double>& flqLearnedCodeRegion = qrcodeLearnInfo.decodedQRCodeInformation.flqRegion;
			CFLString<wchar_t>& flsLearnedCode = qrcodeLearnInfo.decodedQRCodeInformation.flsCode;
			...
			wprintf(L"Code Version : %d\n", qrcodeLearnInfo.i32QRVersion);

			CFLString<wchar_t> flsCodeColor;

			if(qrcodeLearnInfo.eColor == EDataCodeColor_BlackOnWhite)
				flsCodeColor = L"Black On White";
			else if(qrcodeLearnInfo.eColor == EDataCodeColor_WhiteOnBlack)
				flsCodeColor = L"White On Black";

			wprintf(L"Code Color : %s\n", flsCodeColor.GetString());

			CFLString<wchar_t> flsFlip = qrcodeLearnInfo.bFlip ? L"Yes" : L"No";
		...
		int64_t i64Results = modelBasedQRCodeDecoder.GetResultCount();
		...
		for(int64_t i = 0; i < i64Results; ++i)
		{
			...
			if(IsFail(res = modelBasedQRCodeDecoder.GetResultDataRegion(i, flqDecodedCodeRegion)))
			{
			...
			if(IsFail(res = modelBasedQRCodeDecoder.GetResultDecodedString(i, flsDecodedCode)))
			{
		...
		// 동작 이미지 설정 // Set source image
		qrcodeDecoder.SetSourceImage(arrFliImage[EViewList_NormalFind]);
		// 코드 색상 설정 // Sets the code color.
		qrcodeDecoder.SetColorMode(EDataCodeColor_BlackOnWhite);
		// 디코딩 결과 개수 설정 // Sets the number of decoding results.
		qrcodeDecoder.SetDetectingCount(EDataCodeDecoderDetectingCount_All);
		...
		for(int64_t i = 0; i < i64Results; ++i)
		{
			...
			// QR Code Decoder 결과들 중 Data Region 을 얻어옴 // Gets the Data Region from the results of the QR Code decoder.
			if(IsFail(res = qrcodeDecoder.GetResultDataRegion(i, flqDecodedCodeRegion)))
			{
				ErrorPrint(res, "Failed to get data region from the QR code decoder object.");
				continue;
			}
			...
			// QR Code Decoder 결과들 중 Decoded String 을 얻어옴 // Gets the decoded string from the results of the QR Code decoder.
			if(IsFail(res = qrcodeDecoder.GetResultDecodedString(i, flsDecodedCode)))
			{
				ErrorPrint(res, "Failed to get decoded string from the QR code decoder object.");
				continue;
			}
