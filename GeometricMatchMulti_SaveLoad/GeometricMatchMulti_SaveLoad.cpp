...
int main()
{
	...
	CGeometricMatchMulti geometricMatchMultiSave;
	CGeometricMatchMulti geometricMatchMultiLoad;
	...
	do
	{
		...
		for(int64_t i64DataIdx = 0; i64DataIdx < 3; ++i64DataIdx)
		{
			...
			geometricMatchMultiSave.SetLearnImage(fliLearnImage[i64DataIdx]);
			...
			geometricMatchMultiSave.SetFeatureCount();
			...
			geometricMatchMultiSave.SetFeatureFiltering();
			...
			if(IsFail(res = geometricMatchMultiSave.Learn(arrClassName[i64DataIdx])))
			{
			...
			geometricMatchMultiSave.GetLearnedFeature(flfaFeaturePoints);
		...
		if(IsFail(res = geometricMatchMultiSave.Save(L"../../ExampleImages/Matching/Geometric Multi Learn File")))
		{
		...
		if(IsFail(res = geometricMatchMultiLoad.Load(L"../../ExampleImages/Matching/Geometric Multi Learn File")))
		{
		...
		geometricMatchMultiLoad.SetSourceImage(fliFindImage);
		...
		geometricMatchMultiLoad.SetAngleBias(0.);
		...
		geometricMatchMultiLoad.SetAngleTolerance(180.);
		...
		geometricMatchMultiLoad.SetMaxObjectMode(CGeometricMatchMulti::EMaxObjectMode_Total);
		...
		int64_t i64ResultCount = geometricMatchMultiLoad.GetResultCount();
		...
		for(int64_t i = 0; i < i64ResultCount; ++i)
		{
			...
			geometricMatchMultiLoad.GetResult(i, results);
			geometricMatchMultiLoad.GetResultDetectedFeature(i, flfaResultPoints);
