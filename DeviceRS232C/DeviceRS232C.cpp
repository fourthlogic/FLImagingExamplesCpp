...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = rs232C.Initialize()))
		{
		...
		while(true)
		{
			...
			if(i32SelectMode == 1)
			{
				...
				rs232C.Send((uint8_t*)flsCharInput.GetBuffer(), flsCharInput.GetLength());
			}
			else if(i32SelectMode == 2)
			{
				CDeviceSerialPacket packet;
	...
	if(IsFail(res = rs232C.Terminate()))
	{
