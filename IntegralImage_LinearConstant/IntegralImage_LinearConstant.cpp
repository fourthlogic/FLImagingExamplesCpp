...
int main()
{
	...
	do
	{
		...
		// ECalculationMode_LinearConstant : ax + c 형태의 선형 누적합 // ECalculationMode_LinearConstant : Linear sum with constant (ax + c)
		integralImage.SetCalculationMode(CIntegralImage::ECalculationMode_LinearConstant);

		// ax^2 + bx + c 계수 설정(a = 0, b = 2.1, c = 1) // ax^2 + bx + c Setting the coefficient (a = 0, b = 2.1, c = 1)
		integralImage.SetCoefficients(CMultiVar<double>(0, 2.1, 1));
