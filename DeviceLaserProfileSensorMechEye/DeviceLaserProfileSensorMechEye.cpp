...
// 프로파일 센서에서 데이터 취득 이벤트를 받기 위해 CDeviceEventProfileBase 를 상속 받아서 구현
// Inherit from CDeviceEventProfileBase to receive data acquisition events from the profile sensor.
...
public:
	...
	// CDeviceEventProfileEx 소멸자 // Destructor of CDeviceEventProfileEx
	...
	// 프로파일 센서에서 데이터 취득 시 호출 되는 함수 // Callback function called when a data is acquired from the profile sensor.
	virtual void OnAcquisition(const CDeviceProfileBase* pDeviceProfile)
	{
		do
		{
			...
			const CDeviceLaserProfileSensorMechEyeBase* pSensor = dynamic_cast<const CDeviceLaserProfileSensorMechEyeBase*>(pDeviceProfile);
			...
			pSensor->GetAcquired3DData(floData);
...
int main()
{
	...
	// 3D 뷰 선언 // Declare a 3D view.
	CGUIView3DWrap view3D;

	// MechEye 프로파일 센서 선언 // Declare MechEye profile sensor.
	CDeviceLaserProfileSensorMechEye_2_6_0 sensor;
	...
	do
	{
		...
		if(bAutoDetect)
		{
			...
			drReturn = sensor.GetAutoDetectSerialNumbers(flarrSerialNumbers);
		...
		if(bAutoDetect)
			...
			drReturn = sensor.AutoDetect(i32SelectDevice);
		else
			// 시리얼 번호를 설정합니다. // Set the serial number.
			sensor.SetSerialNumber(flsConnection);

		// 프로파일 센서를 초기화 합니다. // Initialize the profile sensor.
		drReturn = sensor.Initialize();

		if(drReturn.IsFail())
		{
			printf("Failed to initialize the profile sensor.\n");
			break;
		}
		...
		drReturn = sensor.Start();

		if(drReturn.IsFail())
		{
			printf("Failed to start the profile sensor.\n");
			break;
		}

		// 프로파일 센서에 소프트웨어 트리거를 발생합니다. // Triggers the profile sensor using a software trigger.
		drReturn = sensor.Trigger();

		if(drReturn.IsFail())
		{
			printf("Failed to trigger the profile sensor.\n");
			break;
		}
