...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliSourceImage.Load(L"../../ExampleImages/Filter/SilverGrass.flif")))
		{
		...
		if(IsFail(res = layerResult1.DrawTextCanvas(&CFLPoint<double>(0, 0), L"RESULT (String)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = layerResult2.DrawTextCanvas(&CFLPoint<double>(0, 0), L"RESULT (Character)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
		...
		randomTextImageGeneratorDL.SetDestinationImage(fliResultImage1);

		// 파라미터 값 설정 // Set parameter value
		randomTextImageGeneratorDL.SetFontSize(20.f, 60.f);
		...
		// Destination 이미지 설정 // Set destination image
		randomTextImageGeneratorDL.SetDestinationImage(fliResultImage2);

		// 파라미터 값 설정 // Set parameter value
		randomTextImageGeneratorDL.SetLabelFigureType(CRandomTextImageGeneratorDL::ELabelFigureType_Character);
