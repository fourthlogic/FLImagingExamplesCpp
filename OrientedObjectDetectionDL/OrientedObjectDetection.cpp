...
{
	COrientedObjectDetectionDL* porientedObjectDetection = (COrientedObjectDetectionDL*)pParam;

	CResult er = porientedObjectDetection->Learn();
	g_bTerminated = true;
...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliLearnImage.Load(L"../../ExampleImages/OrientedObjectDetectionDL/OrientedObjectDetectionSource.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(res = fliValidationImage.Load(L"../../ExampleImages/OrientedObjectDetectionDL/OrientedObjectDetectionSource.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		// orientedObjectDetectionDL 객체 생성 // Create orientedObjectDetection object
		COrientedObjectDetectionDL orientedObjectDetectionDL;
		...
		orientedObjectDetectionDL.SetInterpolationMethod(EInterpolationMethod_Bilinear);
	    // 검증을 진행 할 최소 평균 Cost값 설정 // Set the minimum average cost value at which verification will be triggered
		orientedObjectDetectionDL.SetLearningRequiredAvgCostForValidation(5.f);
		...
		orientedObjectDetectionDL.SetLearningStopCondition(L"mAP >= 0.9");
		// Optimizer의 학습률 설정 // Set learning rate of Optimizer
		optSpec.SetLearningRate(1e-4f);
		optSpec.SetWeightDecay(0.f);
	
		// 설정한 Optimizer를 orientedObjectDetection에 적용 // Apply the Optimizer that we set up to orientedObjectDetection
		...
		autoSaveSpec.SetAutoSavePath(L"model.flood");
		...
		// orientedObjectDetectionDL learn function을 진행하는 스레드 생성 // Create the orientedObjectDetectionDL Learn function thread
		hThread = (HANDLE)_beginthreadex(NULL, 0, LearnThread, (void*)&orientedObjectDetectionDL, 0, nullptr);

		while(!orientedObjectDetectionDL.IsRunning() && !g_bTerminated)
			CThreadUtilities::Sleep(1);

		int32_t i32MaxEpoch = orientedObjectDetectionDL.GetLearningEpoch();
		...
		while(true)
		{
			...
			int32_t i32Epoch = orientedObjectDetectionDL.GetLastEpoch();
			...
			if(i32Epoch != i32PrevEpoch && i32Epoch > 0)
			{
				...
				orientedObjectDetectionDL.GetLearningResultAllHistory(&vctCosts, &vctAvgCosts, &vctmAP, &vctValidationEpoch, &vctRecall, &vctPrecision);
				...
				{
					int32_t i32Step = orientedObjectDetectionDL.GetLearningValidationStep();
					CFLArray<float> flaX;
			...
			if(!orientedObjectDetectionDL.IsRunning() && g_bTerminated)
			{
		...
		orientedObjectDetectionDL.SetInferenceResultImage(fliResultLabelImage);
		// 추론 결과 옵션 설정 // Set the inference result options;
		// Result 결과의 옵션 설정 // Set the option of results
		orientedObjectDetectionDL.SetInferenceResultItemSettings(COrientedObjectDetectionDL::EInferenceResultItemSettings_ClassNum_ClassName_Objectness);

		// 알고리즘 수행 // Execute the algorithm
		if(IsFail(res = orientedObjectDetectionDL.Execute()))
		{
