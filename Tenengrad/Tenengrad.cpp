...
int main()
{
	...
	do
	{
		...
		CTenengrad tenengrad;

		// Source 이미지 1 설정 // Set the source1 image
		tenengrad.SetSourceImage(arrFliImage[EImageType_Source1]);

		//Threshold 설정 //Set Threshold
		tenengrad.SetThreshold(5.0);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(res = tenengrad.Execute()))
		{
		...
		tenengrad.SetSourceImage(arrFliImage[EImageType_Source2]);
		...
		tenengrad.SetSourceImage(arrFliImage[EImageType_Source3]);
