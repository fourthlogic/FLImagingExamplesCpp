...
int main()
{
	...
	do
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = arrFliImage[EType_Source].Load(L"../../ExampleImages/ConvolutionFD/Sun.flif")))
		{
		...
		convolutionFD.SetSourceImage(arrFliImage[EType_Source]);
		// Destination 이미지 설정 // Set destination image 
		convolutionFD.SetDestinationImage(arrFliImage[EType_Destination1]);
		...
		filterGeneratorBandpassFD.SetFilterShape(ImageProcessing::CFilterGeneratorBandpassFD::EFilterShape_Butterworth);
		filterGeneratorBandpassFD.SetDistance(50);
		filterGeneratorBandpassFD.SetDegree(7);

		convolutionFD.SetFilterGenerator(filterGeneratorBandpassFD);
		...
		// Destination 이미지 설정 // Set destination image 
		convolutionFD.SetDestinationImage(arrFliImage[EType_Destination2]);

		// Filter Generator 설정 // Set Filter Generator
		CFilterGeneratorGaussFD filterGeneratorGaussFD;
		filterGeneratorGaussFD.SetSigma1(5);
		filterGeneratorGaussFD.SetSigma2(5);
		...
		if(IsFail(res = arrLayer[EType_Destination1].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination Image(Bandpass)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = arrLayer[EType_Destination2].DrawTextCanvas(&CFLPoint<double>(0, 0), L"Destination Image(Gaussian)", YELLOW, BLACK, 30)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
