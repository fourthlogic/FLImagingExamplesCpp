...
int main()
{
	...
	do
	{
		...
		if((res = operationTrailingOnes.SetSourceImage(fliSourceImage)).IsFail())
		{
		...
		if((res = operationTrailingOnes.SetDestinationImage(fliDestinationImage)).IsFail())
		{
		...
		{
			ErrorPrint(res, L"Failed to execute Operation Trailing Ones.\n");
			break;
		}
		...
		viewSourceImage.SetPixelNumberMode(EPixelNumberMode_Binary);
