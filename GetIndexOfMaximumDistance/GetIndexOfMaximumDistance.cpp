...
int main()
{
	...
	do
	{
		...
		layerDst1.DrawTextCanvas(TPoint<double>(0, 20), L"Index of Maximum Distance", CYAN, BLACK);
		layerDst2.DrawTextCanvas(TPoint<double>(0, 20), L"Index of Maximum Distance", CYAN, BLACK);
		...
		layerDst2.DrawTextImage(flfaArraySrcDepth1.GetCenter(), CFLString<wchar_t>().Format(L"%lld", flvSrcDepth1.v), LIGHTORANGE, BLACK, 12, false, 0, EGUIViewImageTextAlignment_CENTER_CENTER);

		layerDst2.DrawTextImage(flfaArraySrcDepth1.GetAt(flvSrcDepth2.v)->GetCenter(), CFLString<wchar_t>().Format(L"%lld", flvSrcDepth2.v), CYAN, BLACK, 12, false, 0, EGUIViewImageTextAlignment_CENTER_CENTER);
