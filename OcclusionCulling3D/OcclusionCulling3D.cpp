...
int main() {
	...
	do
	{
		...
		if((eResult = floSourceObject.Load(L"../../ExampleImages/OcclusionCulling3D/Cube.ply")).IsFail())
		{
		...
		occlusionCulling3D.SetDestinationObject(fl3DODestinationObject);
		...
		{
			ErrorPrint(eResult, L"Failed to execute OcclusionCulling 3D.");
			break;
		}
