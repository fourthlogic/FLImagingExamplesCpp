...
int main()
{
	...
	do
	{
		...
		if(IsFail(res = fliSrcImage.Load(L"../../ExampleImages/SimplestColorBalance/church.flif")))
		{
		...
		CSimplestColorBalance simplestColorBalance;

		// Source 이미지 설정 // Set the source image
		simplestColorBalance.SetSourceImage(fliSrcImage);

		// Destination 이미지 설정 // Set the destination image
		simplestColorBalance.SetDestinationImage(fliDst1Image);
		...
		simplestColorBalance.SetBalanceMode(CSimplestColorBalance::EBalanceMode_Color);

		// 앞서 설정된 파라미터 대로 알고리즘 수행 // Execute algorithm according to previously set parameters
		if(IsFail(res = simplestColorBalance.Execute()))
		{
		...
		// Destination 이미지 설정 // Set the destination image
		simplestColorBalance.SetDestinationImage(fliDst2Image);

		// Balance Mode 설정 // Set the balance mode
		simplestColorBalance.SetBalanceMode(CSimplestColorBalance::EBalanceMode_Intensity);
		...
		// 색상 파라미터를 EGUIViewImageLayerTransparencyColor 으로 넣어주게되면 배경색으로 처리함으로 불투명도를 0으로 한것과 같은 효과가 있다. // If the color parameter is added as EGUIViewImageLayerTransparencyColor, it has the same effect as setting the opacity to 0 by balance it as a background color.
		...
		if(IsFail(res = layerDst1.DrawTextCanvas(CFLPoint<double>(0, 0), L"Destination1 Image (Color)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}

		if(IsFail(res = layerDst2.DrawTextCanvas(CFLPoint<double>(0, 0), L"Destination2 Image (Intensity)", YELLOW, BLACK, 20)))
		{
			ErrorPrint(res, "Failed to draw text\n");
			break;
		}
