...
int main()
{
	...
	do
	{
		...
		CFLDepthImageReconstruction3D flDepthImageReconstruction3D;

		if((res = flDepthImageReconstruction3D.SetSourceImage(fliSrcImage)).IsFail())
			break;
		if((res = flDepthImageReconstruction3D.SetDestinationHeightMapImage(fliDstImage)).IsFail())
			break;
		if((res = flDepthImageReconstruction3D.SetDestinationTextureImage(&fliTxtImage)).IsFail())
			break;
		if((res = flDepthImageReconstruction3D.SetDestinationObject(floDstObject)).IsFail())
			break;

		if((res = flDepthImageReconstruction3D.SetBackgroundPageIndex(1.00)).IsFail())
			break;
		if((res = flDepthImageReconstruction3D.SetDifferentialMethod(CFLDepthImageReconstruction3D::EDifferentialMethod_DoGY)).IsFail())
			break;
		...
		if((res = flDepthImageReconstruction3D.SetSigmaRange(0.05)).IsFail())
			break;
		if((res = flDepthImageReconstruction3D.SetDirection(CFLDepthImageReconstruction3D::EDirection_BottomToTop)).IsFail())
			break;
