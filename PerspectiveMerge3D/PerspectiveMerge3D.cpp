...
int main()
{
	...
	do
	{
		...
		perspectiveMerge3D.SetEulerSequence(EEulerSequence_Extrinsic_ZXY);
		...
		perspectiveMerge3D.AddSourceObject(&fl3DObjectSrc, tpPosition, tpRotation);
		perspectiveMerge3D.AddSourceObject(&fl3DObjectSrc2, tpPosition2, tpRotation2);

		// Destination 객체 설정 // Set the destination object
		perspectiveMerge3D.SetDestinationObject(fl3DObjectDst);
