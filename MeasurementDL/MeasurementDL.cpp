...
{
	CMeasurementDL* pMeasurement = (CMeasurementDL*)pParam;

	CResult er = pMeasurement->Learn();
	g_bTerminated = true;
...
int main()
{
	...
	do 
	{
		...
		// 이미지 로드 // Loads image
		if(IsFail(res = fliLearnImage.Load(L"../../ExampleImages/MeasurementDL/Shapes.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}

		if(IsFail(res = fliValidationImage.Load(L"../../ExampleImages/MeasurementDL/Shapes.flif")))
		{
			ErrorPrint(res, "Failed to load the image file.\n");
			break;
		}
		...
		measurementDL.SetLearningMaxEdgeRecognitionThickness(5.f);
		
		// 설정한 Optimizer를 Measurement에 적용 // Apply the Optimizer that we set up to Measurement
		...
		autoSaveSpec.SetAutoSavePath(L"model.flmsdl");
		...
		hThread = (HANDLE)_beginthreadex(NULL, 0, LearnThread, (void*)&measurementDL, 0, nullptr);

		while(!measurementDL.IsRunning() && !g_bTerminated)
			CThreadUtilities::Sleep(1);

		int32_t i32MaxEpoch = measurementDL.GetLearningEpoch();
		...
		while(true)
		{
			...
			int32_t i32MaxIteration = measurementDL.GetActualMiniBatchCount();
			// 마지막 미니 배치 반복 횟수 받기 // Get the last number of mini batch iterations
			int32_t i32Iteration = measurementDL.GetLearningResultCurrentIteration();
			// 마지막 학습 횟수 받기 // Get the last epoch learning
			int32_t i32Epoch = measurementDL.GetLastEpoch();
			...
			if(i32Epoch != i32PrevEpoch && i32Iteration == i32MaxIteration && i32Epoch > 0)
			{
				...
				float f32ValidationMeanIoU = measurementDL.GetLearningResultLastMeanIoU();
				...
				measurementDL.GetLearningResultAllHistory(&vctCosts, &vctVadliationPixelAccuracy, &vctMeanIoU, &vctVadliationPixelAccuracyZE, &vctMeanIoUZE, &vctMeanAP, &vctValidationEpoch);
				...
				{
					int32_t i32Step = measurementDL.GetLearningValidationStep();
					CFLArray<float> flaX;
			...
			if(!measurementDL.IsRunning() && g_bTerminated)
			{
		...
		measurementDL.SetInferenceResultImage(fliResultLabelImage);

		// 알고리즘 수행 // Execute the algorithm
		if(IsFail(res = measurementDL.Execute()))
		{
