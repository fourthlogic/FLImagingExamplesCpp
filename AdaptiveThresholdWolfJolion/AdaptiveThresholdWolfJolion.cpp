...
int main()
{
	...
	do
	{
		...
		CAdaptiveThresholdWolfJolion adaptiveThresholdWolfJolion;

		// Source 이미지 설정 // Set source image 
		adaptiveThresholdWolfJolion.SetSourceImage(arrFliImage[EImageType_Source]);

		// Destination 이미지 설정 // Set destination image
		adaptiveThresholdWolfJolion.SetDestinationImage(arrFliImage[EImageType_Destination]);

		// 커널 사이즈 입력 // Set kernel size
		adaptiveThresholdWolfJolion.SetKernel(27, 27);

		// 민감도 설정 // Set Sensitivity
		adaptiveThresholdWolfJolion.SetSensitivity(0.1);

		// 임계값 옵셋 설정 // Set threshold offset 
		adaptiveThresholdWolfJolion.SetThresholdOffset(5);
