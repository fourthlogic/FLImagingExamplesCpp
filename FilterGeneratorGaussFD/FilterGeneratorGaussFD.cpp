...
int main()
{
	...
	do
	{
		...
		// Source 이미지 설정 // Set source image 
		filterGeneratorGaussFD.SetSourceImage(fliISrcImage);

		// Destination 이미지 설정 // Set destination image
		filterGeneratorGaussFD.SetDestinationImage(fliIFilterImage);

		// 정밀도 설정 // Set Accuracy
		filterGeneratorGaussFD.SetAccuracy(EFloatingPointAccuracy_Bit32);

		// 필터 타입 설정 // set Filter type
		filterGeneratorGaussFD.SetType(CFilterGeneratorGaussFD::EFilterBaseFDType_FFT_Shift);
